...
- **PySpin**: vendored as a wheel under [`vendor/spinnaker_python-4.3.0.190-cp310-cp310-win_amd64.whl`](../vendor/spinnaker_python-4.3.0.190-cp310-cp310-win_amd64.whl), registered via `[tool.uv.sources]` in `pyproject.toml`. Pinned to Python 3.10 + numpy<2 (Spinnaker 4.3 ships only cp310 wheels and was built against the NumPy 1.x ABI).
...
- **Protocol**: ASCII over 57600 8N1; addressed commands prefixed with controller index (`1` for first/master), terminated CRLF. `1VE?` = firmware version, `1ID?` = stage ID, `1TS` = state, `1TP` = position, `1RS` = reset. Full command set in the [Command Interface Manual](../manuals/newport-smc100-command-interface.pdf).
- **State machine** (printed on the box): `CONFIG` ↔ `AUTO CONFIG` → `NOT REFERENCED` → `HOMING` → `READY` ↔ `MOVING` / `JOGGING` / `DISABLE`. Faults: `ERROR FE`, `MM0/MM1 ERROR FE`, `HARDWARE FAULT`. Detailed in §5 (Programming) of the [User's Manual](../manuals/newport-smc100-user-manual.pdf).
...
- **Probe script**: [`probe_smc100.py`](../probe_smc100.py) — sends `1VE?` and reports whether the controller replies. Useful for cabling iteration.
...
- **Manuals**: [User's Manual](../manuals/newport-smc100-user-manual.pdf) (EDH0206En2060, 02/25 — install, wiring, state machine, ESP stage configuration), [Command Interface Manual](../manuals/newport-smc100-command-interface.pdf) (EDH0311En1023, 12/21 — `Newport.SMC100.CommandInterface.dll` reference, but the ASCII command names match what goes over RS-232).
...
- **Protocol**: Modbus RTU at 19200 8N1, slave ID 1 by default. 32-bit IEEE floats span two consecutive holding registers. Manuals in [`manuals/`](../manuals/): [M5458 Modbus interface](../manuals/omega-platinum-m5458-modbus.pdf) (register map + enums) and [M5451 controller user guide](../manuals/omega-platinum-m5451-user-manual.pdf) (front-panel menus, OPER modes including M.CNt manual output / M.INP manual input).
